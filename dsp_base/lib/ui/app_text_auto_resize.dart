...
class AppTextAutoResize extends StatefulWidget {
  /// Creates a [AppTextAutoResize] widget.
  ///
  ...
  /// Creates a [AppTextAutoResize] widget with a [TextSpan].
  const AppTextAutoResize.rich(
  ...
  ///
  /// This allows you to find the actual `Text` widget built by `AppTextAutoResize`.
  final Key? textKey;
  ...
  /// Synchronizes the size of multiple [AppTextAutoResize]s.
  ///
  /// If you want multiple [AppTextAutoResize]s to have the same text size, give all
  /// of them the same [AutoSizeGroup] instance. All of them will have the
  /// size of the smallest [AppTextAutoResize]
  ...
  ///
  /// ```dart
  /// AppTextAutoResize(r'$$', semanticsLabel: 'Double dollars')
  /// ```
  ...
  @override
  State<AppTextAutoResize> createState() => _AppTextAutoResizeState();
}

class _AppTextAutoResizeState extends State<AppTextAutoResize> {
  @override
  void initState() {
    super.initState();
  ...
  @override
  void didUpdateWidget(AppTextAutoResize oldWidget) {
    ...
    return LayoutBuilder(
      builder: (context, size) {
        ...
        if (style!.fontSize == null) {
          style = style.copyWith(fontSize: AppTextAutoResize._defaultFontSize);
        }
        ...
        if (widget.overflowReplacement != null && !textFits) {
          return widget.overflowReplacement!.apply(widget.modifier);
        } else {
          return text.apply(widget.modifier);
        }
      },
    );
  }
  ...
  List<Object> _calculateFontSize(
    BoxConstraints size,
    TextStyle? style,
    int? maxLines,
  ) {
    ...
    if (presetFontSizes == null) {
      final num defaultFontSize = style!.fontSize!.clamp(
        widget.minFontSize,
        widget.maxFontSize,
      );
...
class AutoSizeGroupBuilder extends StatefulWidget {
  ...
  const AutoSizeGroupBuilder({required this.builder, super.key}) : super();
...
class AutoSizeGroup {
  final _listeners = <_AppTextAutoResizeState, double>{};
  var _widgetsNotified = false;
  double _fontSize = double.infinity;

  void _register(_AppTextAutoResizeState text) {
    _listeners[text] = double.infinity;
  }

  void _updateFontSize(_AppTextAutoResizeState text, double maxFontSize) {
  ...
  void _remove(_AppTextAutoResizeState text) {
