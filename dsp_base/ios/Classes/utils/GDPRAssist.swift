//
// GDPRAssist.swift
// Helper class for GDPR consent checking based on IAB TCF v2 framework
//

import Foundation

enum GDPRAssist {
    static func isGDPR() -> Bool {
    ...
    static private func hasAttribute(input: String, index: Int) -> Bool {
        return input.count >= index && String(Array(input)[index - 1]) == "1"
    }

    // Check if consent is given for a list of purposes
    static private func hasConsentFor(_ purposes: [Int], _ purposeConsent: String, _ hasVendorConsent: Bool) -> Bool {
        return purposes.allSatisfy { i in hasAttribute(input: purposeConsent, index: i) } && hasVendorConsent
    }

    // Check if a vendor either has consent or legitimate interest for a list of purposes
    static private func hasConsentOrLegitimateInterestFor(
        _ purposes: [Int],
